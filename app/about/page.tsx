...
export const metadata = {
  title: "회사소개",
  description: "식스 마사지 운영사와 사업자 정보를 안내합니다."
};

export default function AboutPage() {
  return (
    <>
      <PageHero eyebrow="About" title="식스 마사지 소개" description="지정 운영지역 중심의 방문 마사지 예약 안내 서비스를 제공합니다." />
      <section className="section" id="business">
        <div className="prose-panel container max-w-3xl rounded-md bg-white p-8 leading-8 shadow-sm">
          ...
          <p className="mt-4">
            식스 마사지 사이트의 콘텐츠는 운영지역과 전화 상담 절차를 기준으로 작성합니다. 지역 페이지는 실제 예약 가능 여부를 확인하려는
            사용자를 위한 전환용 안내이며, 매거진은 직장인 피로 관리, 가족 공간 준비, 지역별 웰니스 생활 팁처럼 정보 제공 목적의 글로
            분리합니다. 같은 문장에 지역명만 바꿔 넣는 방식은 사용하지 않는 것을 원칙으로 합니다.
          </p>
          <p className="mt-4">
            작성 책임은 식스 마사지 운영팀에 있으며, 각 문서는 운영지역, 상담 흐름, 이용 전 확인사항, 금지 표현 기준을 검토한 뒤
